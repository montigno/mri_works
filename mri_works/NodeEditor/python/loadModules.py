...
class getlistModules:
    ...
    def __init__(self, rep):
        ...
        for name in os.listdir(modules):
            if name.endswith(".py"):
                filePy = 'NodeEditor.modules.'+rep+'.'+str(name.replace('.py',''))
...
#                     print(nameClass," : ",inspect.getcomments(obj))
                    if inspect.isclass(obj):
                        try:
                            ...
                            if 'NodeEditor.python.configStandalone' in self.objectsInModul:
                                sch = src.find('getPathConfig(')
                                if sch!=-1:
                                    tmpSrc = src[src.index('getPathConfig(')+14:]
                                    tmpSrc = tmpSrc[0:tmpSrc.index(')')]
                                    if 'self' not in tmpSrc:
                                        tmpSrc=   tmpSrc[1:-1]
                                        self.listStand.append(tmpSrc)
                                self.objectsInModul.remove('NodeEditor.python.configStandalone')
                            ...
                            if obj.__module__.find('modules.sources') ==-1:
                                ...
                                listClass.append((nameClass,listArgs[0],listArgs[3],listOrderClass,listTypeOut))
...
#                             print('no class',nameClass)
                self.category.append((name.replace('.py',''),list(listClass)))
                self.category.sort()

    def findClassOrder(self,txtClass):
        list=[]
        for line in txtClass.splitlines():
            if 'def ' in line and not 'def __init__' in line and not 'def _' in line:
