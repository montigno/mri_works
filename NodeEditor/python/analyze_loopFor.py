     ...
     ####### search value ForLoop inputs and outputs #########################
        for keyAe, valueAe in listArrowExtern.items():
            ...
            if keyF + ':in' in tmp1 or keyF+':val' in tmp1:
                self.listConnectIn.append(tmp1 + '=' + valueAe[0:valueAe.index('#Node#')])
#             tmp2 = valueAe
...
#                  self.listConnectOut.append(tmp1+'='+valueAe[0:valueAe.index('#Node#')])
        ...
        for keyAi, valueAi in listArrowIntern.items():
            ...
            if keyF + ':out' in tmp1 and unittmp1 != unittmp2: # if unittmp1 == unittmp2 => link direct in loopIf
    ...
    ######### search direct link in loop If ###############################
        ...
        if len(valueF) == 5 : #in case of loopIf
            tmpVal = valueF[4]
            for el in tmpVal:
    ...
    ####### search sequence of stacks #####################################
        ...
        for keyAi, valueAi in listArrowIntern.items():
            ...
            if  keyF not in de and \
                keyF not in df and \
                (de in self.listBlock.keys() or de in self.listModul.keys()) and \
...
#         print('listArrowIntern',listArrowIntern)
        ...
        tmp = (list(set(listBlockIntern) - set(listBlockStart) - set(listBlockStop)))
        for lb in tmp:
...
#              listBlockRemaining = (list(set(tmp)-set(listBlockStart)))
            listBlockRemaining = (list(set(listBlockIntern) - set(listBlockStart) - set(listBlockStop)))
...
#         print('listBlockStop LoopFor',listBlockStop)
    ...
    ####################### create list of Block to execute in order###########
        ...
        if len(self.listBlockExecution) > 1:
            self.listBlockExecution = ['ThreadOn'] + self.listBlockExecution + ['ThreadOff']
