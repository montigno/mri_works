...
#         print(listDynamicValue)
        ...
        if modul:
            for ls in listModul.keys():
                file = os.path.join(QDir.currentPath(),'NodeEditor','submodules',modul[0]+'.mod')
                f = open(file,'r')
                txt = f.read()
                f.close()
        ...
        for execution in listBlockExecution:
            if 'ThreadOn' in execution:
                ...
                threadcurrent=False
            if not 'M' in execution and not 'Thread' in execution and not 'F' in execution and not 'I' in execution:
...
#                         if textEditor:
#                             textEditor.append("<span style=\" font-size:10pt; font-weight:600; color:#cc0000;\" > Pipeline SubMod Execution stopped <br>" +execution+' : '+str(e)+"</span>")
                        return
                ...
                try:
                    a=executionSubmod(listModExecution[execution],listDynamicValueSub2,None,listModul[execution])
                    for uj in a.getOutValues().keys():
                        listDynamicValue[uj]=a.getOutValues()[uj]
                except Exception as e:
#                     if textEditor:
#                         textEditor.append("<span style=\" font-size:10pt; font-weight:600; color:#cc0000;\" > Pipeline SubMod Execution SubMod stopped <br>" +execution+' : '+str(e)+"</span>")
                    return
                ...
                try:
                    if 'm' in execution:
                        executionFor_proc(listForExecution[execution],listDynamicValueFor,txt)
                    else:
                        a,b,c,d,e,f,g = ForLoopInfo(listForExecution[execution],txt).getListInfo()
                        ...
                        for ele in range(lengthEnter):
                            ...
                            try:
                                es=executionSubmod(listForExecution[execution],listDynamicValueSub.copy(),textEditor,modul)
                                ...
                                textEditor.append("<span style=\" font-size:10pt; font-weight:600; color:#cc0000;\" > Pipeline Execution For Loop  stopped : "+str(e)+"</span>")
                                return
