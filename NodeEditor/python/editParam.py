...
from PyQt5.QtWidgets import QVBoxLayout, QHBoxLayout, QDialog, QLabel, QLineEdit, \
    QPushButton, QWidget, QGroupBox, QComboBox, QScrollArea
...
#                 self.listVal.append(i.currentText())
                ...
                if self.listField[self.label[index]]=='path' and tmp not in 'path':
                    if not os.path.exists(tmp):
                        self.info.setText("<span style=\" font-size:10pt;color:#cc0000;\" > error :"+self.label[index]+
                                      " does't seems exist </span>")
                        return
                    else:                  
                        self.listVal.append(tmp)
    
                elif not DefinitType(tmp).returntype() == self.listField[self.label[index]]:
                    self.info.setText("<span style=\" font-size:10pt;color:#cc0000;\" > error :"+self.label[index]+" must be "+
                                      self.listField[self.label[index]]+"</span>")
                    return
                else:
                    self.listVal.append(tmp)
        self.close()
...
#                 print('detect enumerate !')
                typVar='enumerate'
                typVal='str'
#             typVal=self.isPath(str(list(eval(self.var))[0][1]))
